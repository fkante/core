...
- Database: postgresql://postgres:postgres@localhost:5433/app
...
- Frontend served by the TanStack Start SSR Node server on port 3000 (exposed as 8080)
- Optimized logging
...
1. **Builder stage**: Installs dependencies and builds the app (`vite build` → `.output` via the nitro plugin). `VITE_API_BASE_URL` and `VITE_GOOGLE_CLIENT_ID` are passed as build args and baked into the client bundle.
2. **Production stage**: Runs the TanStack Start SSR Node server (`node .output/server/index.mjs`). Because the frontend is SSR (server functions, `beforeLoad` auth gating, the tRPC route), the client must reach the backend cross-origin via the build-time `VITE_API_BASE_URL`.
...
Migrations live in `apps/backend/drizzle/` (generated by drizzle-kit). `scripts/start.sh` and `scripts/deploy.sh` run migrate + seed against the running Postgres before starting the app containers:

```bash
pnpm --filter backend db:generate   # schema change → new SQL migration
pnpm --filter backend db:migrate    # apply migrations
pnpm --filter backend db:seed       # bootstrap admin + example notes (idempotent)
```

For the host-based workflow, `pnpm bootstrap` (from the repo root) brings up Postgres and runs migrate + seed in one step.
